"use strict";
/* GAIA TERMINAL — an ASCII-art edition of the GAIA planetary sim.
   Same SimEarth-spirited mechanics as the main game; the world is drawn entirely in
   CP437 glyphs and sixteen CGA colours on a Canvas2D "terminal", built for touch and
   tuned for both phones (portrait) and iPad (landscape). Vanilla JS, no build step. */
...
const CLASS_ICON=["·","🦠","🧫","🪼","🦐","🐟","🐸","🦎","🦕","🐘","🧠"];
/* per-class CP437-ish creature glyph, scattered over inhabited tiles when zoomed in */
const CLASS_GLYPH=["·","·",":","j","x","f","y","r","D","m","@"];
...
/* ============================================================================
   CP437 / CGA TERMINAL RENDERER
   The whole world is drawn as a grid of glyphs in sixteen CGA colours. Each cell
   resolves (per active layer) to a {ch, fg, bg} — a character and two palette
   indices. When zoomed out so cells are tiny, we drop to solid colour blocks (a
   CGA "satellite" map); zoomed in, glyphs and scattered creature letters read.
   ========================================================================== */
const CGA=['#05050c','#0000aa','#00aa00','#00aaaa','#aa0000','#aa00aa','#aa5500','#aaaaaa',
           '#555577','#5555ff','#55ff55','#55ffff','#ff5555','#ff55ff','#ffff55','#ffffff'];
const CGA_RGB=[[5,5,12],[0,0,170],[0,170,0],[0,170,170],[170,0,0],[170,0,170],[170,85,0],[170,170,170],
               [85,85,119],[85,85,255],[85,255,85],[85,255,255],[255,85,85],[255,85,255],[255,255,85],[255,255,255]];
const K=0,BLU=1,GRN=2,CYN=3,RED=4,MAG=5,BRN=6,GRY=7,DGY=8,LBL=9,LGR=10,LCY=11,LRD=12,LMG=13,YEL=14,WHT=15;
function nearestCGA(rgb){let bi=15,bd=1e9;for(let i=1;i<16;i++){const d=(rgb[0]-CGA_RGB[i][0])**2+(rgb[1]-CGA_RGB[i][1])**2+(rgb[2]-CGA_RGB[i][2])**2;if(d<bd){bd=d;bi=i;}}return bi;}
const LIFE_CGA=LIFE_COLORS.map(nearestCGA);
const DENSITY=['∙',':','+','*','#'];        // biomass ramp
function densCh(v){return DENSITY[clamp(Math.floor(v*5),0,4)];}

/* --- per-layer cell styling --- */
function terrainStyle(i){
  const a=alt[i],t=temp[i],v=bio[i],b=biome[i];
  if(a<G.seaLevel){
    if(b===SEAICE) return {ch:'░',fg:LCY,bg:BLU};
    const deep=clamp(-a,0,1);
    if(cloud[i]>0.45) return {ch:'▒',fg:LBL,bg:BLU};
    return deep>0.45?{ch:'≈',fg:BLU,bg:K}:{ch:'~',fg:LBL,bg:BLU};
  }
  if(a>0.55&&t>-4) return {ch:'▲',fg:a>0.82?WHT:GRY,bg:K};   // mountains, snow-capped high up
  switch(b){
    case GLACIER: return {ch:'▲',fg:WHT,bg:K};
    case TUNDRA:  return {ch:'·',fg:GRY,bg:K};
    case TAIGA:   return {ch:'♠',fg:GRN,bg:K};
    case DESERT:  return {ch:'~',fg:YEL,bg:K};
    case GRASS:   return v<0.06?{ch:'·',fg:BRN,bg:K}:{ch:'"',fg:LGR,bg:K};
    case SAVANNA: return v<0.06?{ch:'·',fg:BRN,bg:K}:{ch:'ˌ',fg:BRN,bg:K};
    case FOREST:  return v<0.06?{ch:'·',fg:BRN,bg:K}:{ch:'♣',fg:GRN,bg:K};
    case RAIN:    return v<0.06?{ch:'·',fg:BRN,bg:K}:{ch:'♣',fg:LGR,bg:K};
    case WETLAND: return {ch:'≈',fg:CYN,bg:K};
    default:      return {ch:'·',fg:DGY,bg:K};
  }
}
function heatStyle(i){
  const t=clamp(temp[i],-40,55),u=(t+40)/95;let fg,ch;
  if(u<0.20){fg=BLU;ch=':';}
  else if(u<0.38){fg=LBL;ch=':';}
  else if(u<0.55){fg=CYN;ch='+';}
  else if(u<0.68){fg=GRN;ch='*';}
  else if(u<0.82){fg=YEL;ch='#';}
  else{fg=LRD;ch='#';}
  return {ch,fg,bg:K};
}
function lifeStyle(i){
  const v=bio[i],k=lcl[i];
  if(v<0.02||k<=0) return {ch:'·',fg:alt[i]<G.seaLevel?BLU:DGY,bg:K};
  return {ch:densCh(clamp(v,0,1)),fg:LIFE_CGA[k]||LGR,bg:K};
}
function airStyle(i){
  const cl=cloud[i],m=moist[i];
  if(cl>0.55) return {ch:'▓',fg:WHT,bg:K};
  if(cl>0.28) return {ch:'▒',fg:GRY,bg:K};
  if(alt[i]<G.seaLevel) return {ch:'~',fg:BLU,bg:K};
  if(m>0.6) return {ch:'░',fg:LBL,bg:K};
  if(m>0.3) return {ch:'·',fg:DGY,bg:K};
  return {ch:' ',fg:DGY,bg:K};
}
function styleAt(i){
  switch(G.layer){case 'climate':return heatStyle(i);case 'life':return lifeStyle(i);
    case 'air':return airStyle(i);default:return terrainStyle(i);}
}
...
/* CAMERA — the viewport into the world. zoom=1 shows the whole planet; higher zoom
   moves the eye closer. The world wraps horizontally, so the camera pans freely
   east/west and is clamped north/south. (Shared math with the pixel edition.) */
const DEFAULT_ZOOM=4.5,MIN_ZOOM=1,MAX_ZOOM=16;
...
const GLYPH_MIN=8,CREATURE_MIN=17;
function fract(v){return v-Math.floor(v);}
/* Scatter the dominant family's letter over inhabited tiles when zoomed in — the
   ASCII echo of the pixel edition's fauna sprites. Density rises with biomass,
   gated by each cell's fixed seed so the scatter is stable and organic. */
function drawCreatures(x0,x1,y0,y1,left,top,ppcX,ppcY,cs){
  const alpha=clamp((cs-CREATURE_MIN)/(CREATURE_MIN*0.7),0,1);
  pctx.save();pctx.globalAlpha=alpha;pctx.textAlign='center';pctx.textBaseline='middle';
  pctx.font='bold '+Math.round(cs*0.66)+'px "VT323",ui-monospace,monospace';
  pctx.shadowColor='rgba(0,0,0,0.7)';pctx.shadowBlur=Math.max(1,cs*0.08);
  for(let wy=y0;wy<y1;wy++){const sy=(wy-top)*ppcY;
    for(let wx=x0;wx<x1;wx++){
      const i=wy*W+(((wx%W)+W)%W),k=lcl[i],b=bio[i];
      if(k<=0||b<=0.06) continue;
      if(seed[i]>=clamp((b-0.06)/0.55,0,1)*0.6) continue;
      const jx=(seed[i]-0.5)*0.30,jy=(fract(seed[i]*7.31)-0.5)*0.30;
      pctx.fillStyle=CGA[LIFE_CGA[k]||LGR];
      pctx.fillText(CLASS_GLYPH[k]||'·',(wx-left+0.5+jx)*ppcX,(wy-top+0.5+jy)*ppcY);
    }}
  pctx.restore();
}
function render(){
  ...
  const ppcX=pw/vw,ppcY=ph/vh,cs=Math.min(ppcX,ppcY);
  pctx.setTransform(1,0,0,1,0,0);
  pctx.imageSmoothingEnabled=false;
  pctx.fillStyle=CGA[K];pctx.fillRect(0,0,pw,ph);
  const x0=Math.floor(left),x1=Math.ceil(left+vw),y0=Math.max(0,Math.floor(top)),y1=Math.min(H,Math.ceil(top+vh));
  const glyph=cs>=GLYPH_MIN;
  if(glyph){pctx.textAlign='center';pctx.textBaseline='middle';
    pctx.font=Math.round(cs*0.96)+'px "VT323",ui-monospace,monospace';}
  const bw=Math.ceil(ppcX)+1,bh=Math.ceil(ppcY)+1;
  for(let wy=y0;wy<y1;wy++){const sy=(wy-top)*ppcY;
    for(let wx=x0;wx<x1;wx++){
      const i=wy*W+(((wx%W)+W)%W),st=styleAt(i),sx=(wx-left)*ppcX;
      if(!glyph){                                    // solid CGA block — the zoomed-out map
        pctx.fillStyle=CGA[st.fg];pctx.fillRect(Math.floor(sx),Math.floor(sy),bw,bh);
      }else{
        if(st.bg){pctx.fillStyle=CGA[st.bg];pctx.fillRect(Math.floor(sx),Math.floor(sy),bw,bh);}
        if(st.ch!==' '){pctx.fillStyle=CGA[st.fg];pctx.fillText(st.ch,sx+ppcX/2,sy+ppcY/2+ppcY*0.06);}
      }
    }}
  if(glyph&&cs>=CREATURE_MIN&&(G.layer==='terrain'||G.layer==='life'))
    drawCreatures(x0,x1,y0,y1,left,top,ppcX,ppcY,cs);
  if(G.civ.on){                                       // cities — bright houses
    pctx.textAlign='center';pctx.textBaseline='middle';
    pctx.font=(glyph?Math.round(cs*0.96):Math.max(6,Math.round(cs*1.6)))+'px "VT323",ui-monospace,monospace';
    for(const ct of G.civ.cities){
      let sx=(ct.x-left),sy=(ct.y-top);
      sx=((sx%W)+W)%W; if(sx>vw) sx-=W;              // wrap the city into view
      if(sx<-1||sx>vw+1||sy<-1||sy>vh+1) continue;
      pctx.fillStyle=CGA[YEL];
      if(glyph) pctx.fillText('⌂',(sx+0.5)*ppcX,(sy+0.5)*ppcY+ppcY*0.06);
      else{pctx.fillRect(Math.floor(sx*ppcX),Math.floor(sy*ppcY),bw,bh);}
    }
  }
  updateLifeKey();
}
...
const LAYER_LEGEND={terrain:'BIOMES',life:'LIFE',climate:'',air:''};
let lifeKeySig='';
/* Map legend, reusing the #lifeKey window: life families on the LIFE map, and the
...
function updateLifeKey(){const key=document.getElementById('lifeKey');
  ...
  if(G.layer==='life'){
    ...
    let html='';for(let c=10;c>=1;c--){if(!present[c])continue;const col=LIFE_COLORS[c];
      html+=`<span><i style="background:rgb(${col[0]},${col[1]},${col[2]})"></i>${CLASS_GLYPH[c]} ${CLASSES[c]}</span>`;}
...
function updateMonitors(){
  spark('g-temp',G.log.temp,'#ffff55');spark('g-co2',G.log.co2,'#ff55ff');spark('g-o2',G.log.o2,'#55ffff');spark('g-bio',G.log.bio,'#55ff55');
  ...
  const parts=[["N₂",n2,"#5555ff"],["O₂",o2,"#55ffff"],["CO₂",co2pct,"#ff55ff"],["CH₄",ch4,"#55ff55"]];
  ...
  for(let c=1;c<=10;c++){const r=document.createElement('div');r.className='rung'+(c<=G.maxClass?' reached':'')+(c===G.maxClass?' current':'');
    r.innerHTML=`<span class="rn">${CLASS_GLYPH[c]}</span><span>${CLASSES[c]}</span>`;lad.appendChild(r);}
...
/* Responsive shell: wide landscape → three-column iPad layout; otherwise the
   stacked phone layout with a slide-up monitors sheet. */
function updateLayout(){
  const wide=window.innerWidth>=820&&window.innerWidth>window.innerHeight;
  document.body.classList.toggle('wide',wide);
  if(wide){monOpen=false;monitors.classList.remove('open');}
  fit();if(G&&G.started)render();
}
...
function startGame(key){modal.classList.add('hidden');
  ...
  buildFaunaBar();selectTool('inspect');updateLayout();resetCam();render();updateUI();
  toast(SCENARIOS[key].name+" loaded — shape your world");}

G=freshGlobal();updateLayout();
